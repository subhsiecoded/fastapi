...
def get_current_user(token: str = Depends(oauth2_scheme), db:Session= Depends(database.get_db)):
    # we need the get_current_user function although it only calls the verify_acces_token function, this is because the verify_access_token function returns the token data which is the user ID 
    # But this function fetches the user from the database with the ID returned from the verify_access_token function
